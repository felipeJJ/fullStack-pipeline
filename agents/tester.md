...
- The project's **`CLAUDE.md`** — read it first if present (subagents don't inherit it automatically). The overview, domain glossary, permanent decisions and "never" rules are binding context: a test that contradicts a documented decision is a wrong test.
